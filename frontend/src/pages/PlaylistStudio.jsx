    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="fixed inset-0 z-[9999] flex items-end justify-center p-4 sm:items-center bg-black/80 backdrop-blur-sm"
          onClick={onClose}
        >
          <motion.div
          ...
          >
            ...
            <form onSubmit={go} className="space-y-4">
              <div>
                <label className="block mb-1 text-sm font-black uppercase">
                  Name
                </label>
                <input
                  value={name}
                  ...
                  className="w-full p-2.5 font-bold bg-white border-[3px] border-neoBlack shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] focus:shadow-none focus:translate-x-[3px] focus:translate-y-[3px] transition-all outline-none"
                  required
                />
              </div>
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full h-24 p-2.5 font-bold resize-none bg-white border-[3px] border-neoBlack shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] focus:shadow-none focus:translate-x-[3px] focus:translate-y-[3px] transition-all outline-none"
                  required
                />
              </div>
              <button
  ...
  <AnimatePresence>
    {isOpen && (
      ...
      >
        ...
        >
          <h2 className="pb-2 mb-3 text-2xl font-black uppercase border-b-[3px] border-neoBlack text-[#ff0055]">
            Delete Playlist?
          </h2>
          <p className="mb-6 text-sm font-bold">
            Destroy &quot;{playlistName}&quot; permanently? This cannot be
            undone.
          </p>
  ...
  return (
    <div className="pt-4 mt-4 border-t-4 border-neoBlack">
      ...
      <div className="flex gap-2">
        ...
        />
        {searching && (
          <div className="self-center text-xs font-black animate-pulse">
            ...
          </div>
        )}
      </div>
        ...
        {results.map((v) => (
          ...
          >
            <img
              src={v.thumbnail}
              alt=""
              className="object-cover w-12 h-8 border border-neoBlack"
            />
            <span className="flex-1 text-xs font-bold line-clamp-1">
              {v.title}
            </span>
            <button
              onClick={() => addVideo(v._id)}
...
const VideoManager = ({ playlist, onClose, showToast }) => {
  ...
  const removeVideo = async (videoId) => {
    ...
    try {
      const { data } = await api.patch(
        `/playlist/remove/${videoId}/${playlist._id}`,
      );
  ...
  const handleAdded = () => {
    fetchPlaylist();
    showToast("Video added! 🎬", "bg-[#8fff00]");
  };
    ...
    >
      ...
      >
        <div className="p-4 bg-[#8fff00] border-b-[4px] border-neoBlack sticky top-0 z-10">
          ...
          <p className="mt-1 text-sm font-bold text-black opacity-80 line-clamp-1">
            {playlist.name}
          </p>
        </div>
        ...
        <div className="flex-1 p-4">
          {loading ? (
            <div className="space-y-3">
              {[0, 1, 2].map((i) => (
                <div
                  key={i}
                  className="bg-gray-200 border-2 h-14 animate-pulse border-neoBlack"
                />
              ))}
            </div>
          ) : videos.length === 0 ? (
            ...
            >
              <Film
                size={36}
                className="mx-auto mb-3 stroke-[2] text-gray-400"
              />
              ...
              <AnimatePresence>
                {videos.map((video) => (
                  ...
                  >
                    <Link
                      to={`/video/${video._id}`}
                      className="flex-shrink-0 block w-20 overflow-hidden border-[2px] h-12 border-neoBlack"
                    >
                      <img
                        src={video.thumbnail}
                        alt={video.title}
          ...
          <QuickAdd
            ...
            existingIds={videos.map((v) => v._id)}
            onAdded={handleAdded}
          />
        </div>
      </motion.div>
...
export const PlaylistStudio = ({ userId }) => {
  ...
  const fetchPlaylists = useCallback(async (currentTab, uid) => {
    ...
    try {
      ...
      const endpoint =
        currentTab === "discovery" ? "/playlist/all" : `/playlist/user/${uid}`;
  ...
  const handleUpdate = async ({ name, description }) => {
    ...
    setPlaylists((p) =>
      p.map((pl) => (pl._id === targetId ? { ...pl, name, description } : pl)),
    );
    setEditTarget(null);
    try {
      const { data } = await api.patch(`/playlist/${targetId}`, {
        name,
        description,
      });
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      showToast(
        "GRAVITY SHIFTED: PLAYLIST DELETED SUCCESSFULLY",
      ...
      if (status === 403 || status === 404) {
        showToast(
          "SYSTEM FAILURE: YOU DON'T HAVE PERMISSION TO DELETE",
          "bg-neoYellow",
        );
      } else {
