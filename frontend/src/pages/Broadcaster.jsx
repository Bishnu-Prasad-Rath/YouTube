...
import { Track, ConnectionState, VideoPresets, VideoQuality, Room } from "livekit-client";
...
const CustomControls = ({ containerRef, onEndStream }) => {
  ...
  const toggleMic = async () => {
      ...
      if (!isMicEnabled) {
          ...
          await localParticipant.setMicrophoneEnabled(true, { deviceId: selectedMicId || 'default' });
      } else {
...
const MainVideoArea = ({ containerRef, onEndStream }) => {
    ...
    return (
        <div ref={containerRef} className="relative w-full h-full bg-black border-4 border-neoBlack overflow-hidden shadow-[inset_0px_0px_20px_rgba(0,0,0,0.8)] group">
      ...
      <div className="flex items-center justify-between mb-6">
          ...
          {isLive && (
              <button
                ...
                className="neo-btn bg-neoRed text-white font-black text-lg uppercase border-4 border-neoBlack shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:bg-red-500 py-2 px-8 active:translate-x-1 active:translate-y-1 active:shadow-none tracking-widest"
              >
                End Stream
              </button>
          )}
...
>
              ...
              <div className="w-full lg:w-[75%] h-[50vh] lg:h-full p-4 lg:p-6 flex flex-col relative z-0">
                  <MainVideoArea containerRef={videoContainerRef} onEndStream={handleEndStream} />
              </div>
