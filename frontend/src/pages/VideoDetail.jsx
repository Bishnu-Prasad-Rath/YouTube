...
export const VideoDetail = () => {
  ...
  const { isSubscribed, subscribersCount, toggleSubscription, loading } =
    useSubscription(video?.owner?._id, initialSubState, initialSubCount);
    ...
    <div className="max-w-[1600px] mx-auto px-6 pb-12 flex flex-col lg:flex-row gap-8 pt-4">
      ...
      <div className="flex-none lg:w-[65%] xl:w-[70%]">
        ...
        <div
          ...
          className="relative p-0 mb-6 border-4 neo-card border-neoBlack shadow-neo group bg-neoBlack"
        >
          <video
            ref={videoRef}
            src={video.videoFile}
          ...
          {/* Controls Overlay */}
          <div
            className={`absolute bottom-0 left-0 right-0 p-4 transition-opacity duration-300 flex flex-col justify-end bg-gradient-to-t from-black/80 via-black/40 to-transparent ${isPlaying ? "opacity-0 group-hover:opacity-100" : "opacity-100"}`}
          >
            {/* Progress Bar */}
            <div
            ...
            <div className="flex items-center justify-between text-white">
              <div className="flex gap-4">
                <button
                  onClick={togglePlay}
                  className="p-2 border-2 border-black bg-neoGreen text-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:translate-y-[2px] active:translate-x-[2px] active:shadow-none hover:bg-[#9eff00] transition-colors"
                >
                  {isPlaying ? (
        ...
        <div className="relative flex items-center justify-between p-4 mb-6 overflow-hidden border-4 bg-neoWhite border-neoBlack shadow-neo">
          <div className="relative z-10 flex items-center gap-4">
            <Link to={`/u/${video.owner?.username}`}>
              <img
                src={
                  video.owner?.avatar || "https://ui-avatars.com/api/?name=U"
                }
                alt="channel"
                className="w-14 h-14 rounded-full border-4 border-neoBlack bg-neoYellow object-cover shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:scale-105 transition-transform"
              />
            </Link>
            <div>
              <Link
        ...
        <div
          className="border-4 border-neoBlack shadow-neo bg-neoGreen mb-8 p-4 cursor-pointer hover:bg-[#b0ff30] transition-colors overflow-hidden"
        ...
        >
          ...
          >
            <p className="font-bold whitespace-pre-wrap">{video.description}</p>
          </motion.div>
        ...
        <div>
          <h2 className="inline-block px-4 py-2 mb-6 text-2xl font-black uppercase border-4 bg-neoBlue border-neoBlack shadow-neo text-neoWhite">
          ...
          {currentUser ? (
            ...
            <div className="p-4 mb-8 text-lg font-bold text-center uppercase border-4 border-neoBlack bg-neoYellow shadow-neo">
      ...
      <div className="flex flex-col flex-1 gap-6">
        <h2 className="inline-block px-4 py-2 mb-2 text-2xl font-black uppercase border-4 bg-neoYellow border-neoBlack shadow-neo w-max">
          Recommended
        </h2>
        {recLoading ? (
          <>
            {[1, 2, 3].map((_, idx) => (
              <div
                key={idx}
                className="neo-card p-0 overflow-hidden flex flex-col h-[280px] bg-neoWhite border-4 border-neoBlack animate-pulse"
              >
